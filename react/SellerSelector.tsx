...
const SellerSelector: StorefrontFunctionComponent<any> = ({ slug }) => {
  ...
  if (selectedItem) {
    return (
      <div key={slug}>
        <div
          className={`${handles.sellersHeader} flex items-center-s br2 bg-muted-3 hover-bg-muted-3 active-bg-muted-3 c-on-muted-3 hover-c-on-muted-3 active-c-on-muted-3 dib mr3`}
        >
        ...
        {selectedItem.sellers.map((current: any, index: any) => (
          <div
            key={index}
            className={`${handles.sellersInfoBox} bt-0 b--solid-s bw1 b--muted-3 flex br--bottom items-center-s`}
          >
